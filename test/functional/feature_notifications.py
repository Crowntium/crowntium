...
from test_framework.address import ADDRESS_BXCRT1_UNSPENDABLE, keyhash_to_p2pkh
...
class NotificationsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        block_count = 10
        blocks = self.nodes[1].generatetoaddress(block_count, self.nodes[1].getnewaddress() if self.is_wallet_compiled() else ADDRESS_BXCRT1_UNSPENDABLE)
        ...
        if self.is_wallet_compiled():
            ...
            generatesynchronized(self.nodes[0], COINBASE_MATURITY, ADDRESS_BXCRT1_UNSPENDABLE, self.nodes)

            # Generate transaction on node 0, sync mempools, and check for
            # notification on node 1.
            tx1 = self.nodes[0].sendtoaddress(address=ADDRESS_BXCRT1_UNSPENDABLE, amount=1, replaceable=True)
            ...
            self.nodes[0].generatetoaddress(1, ADDRESS_BXCRT1_UNSPENDABLE)
            self.sync_blocks()
            self.expect_wallet_notify([bump1])
            assert_equal(self.nodes[1].gettransaction(bump1)["confirmations"], 1)

            # Generate a second transaction to be bumped.
            tx2 = self.nodes[0].sendtoaddress(address=ADDRESS_BXCRT1_UNSPENDABLE, amount=1, replaceable=True)
