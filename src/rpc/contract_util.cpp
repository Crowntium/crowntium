...
UniValue transactionReceiptToJSON(const CrowntiumTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
UniValue CallToContract(const UniValue& params)
{
    ...
    if(params.size() >= 3){
        CTxDestination crtSenderAddress = DecodeDestination(params[2].get_str());
        if (IsValidDestination(crtSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&crtSenderAddress);
    ...
    for(size_t i = 0; i < list.size(); i++)
    {
        ...
        for(size_t i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                ToCrowntiumAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].get_str().substr(24);
                ToCrowntiumAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
