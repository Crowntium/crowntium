...
const char * const BITCOIN_CONF_FILENAME = "crt.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Crowntium
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Crowntium
    // Mac: ~/Library/Application Support/Crowntium
    // Unix: ~/.crt
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Crowntium";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Crowntium";
#else
    // Unix
    return pathRet / ".crt";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Crowntium Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Crowntium Core Developers";
    }
