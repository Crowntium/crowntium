...
#ifdef _M_IX86
#define _XCRT_DEBUGGER_HOOK _crt_debugger_hook
#else
#define _XCRT_DEBUGGER_HOOK __crt_debugger_hook
#endif
#if _MSC_VER < 1900
extern "C" {_XCRTIMP void __cdecl _XCRT_DEBUGGER_HOOK(int);}
#else
extern "C" {void __cdecl _XCRT_DEBUGGER_HOOK(int); }
#endif
#endif
#endif
...
#if defined(_MSC_VER) && _MSC_VER >= 1400
				// first byte of _XCRT_DEBUGGER_HOOK gets modified in memory by the debugger invisibly, so read it from file
				if (IsDebuggerPresent())
				{
					if (subSectionMemStart <= (byte *)&_XCRT_DEBUGGER_HOOK && (byte *)&_XCRT_DEBUGGER_HOOK < subSectionMemStart + subSectionSize)
					{
						subSectionSize = (byte *)&_XCRT_DEBUGGER_HOOK - subSectionMemStart;
						nextSubSectionStart = (byte *)&_XCRT_DEBUGGER_HOOK - sectionMemStart + 1;
					}
				}
#endif
