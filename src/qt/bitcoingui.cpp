...
#include <qt/crtversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    crtVersionChecker = new CrowntiumVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Crowntium address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and crt: URIs)"));
    ...
    XCRTTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/crttoken"), tr("&XCRT Tokens"), this);
    XCRTTokenAction->setStatusTip(tr("XCRT Tokens (send, receive or add Tokens in list)"));
    XCRTTokenAction->setToolTip(XCRTTokenAction->statusTip());
    XCRTTokenAction->setCheckable(true);
    XCRTTokenAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
    tabGroup->addAction(XCRTTokenAction);
    ...
    connect(XCRTTokenAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(XCRTTokenAction, SIGNAL(triggered()), this, SLOT(gotoTokenPage()));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Crowntium addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Crowntium addresses"));
    ...
    openAction->setStatusTip(tr("Open a crt: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Crowntium command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->addAction(XCRTTokenAction);
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && crtVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(XCRT_RELEASES, XCRT_RELEASES);
            QString message(tr("New version of Crowntium wallet is available on the Crowntium source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    XCRTTokenAction->setEnabled(enabled);
...
{
    XCRTTokenAction->setChecked(true);
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Crowntium network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
