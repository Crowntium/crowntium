...
#define paternVersion "crt-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

CrowntiumVersionChecker::CrowntiumVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

CrowntiumVersionChecker::~CrowntiumVersionChecker()
{

}

bool CrowntiumVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> CrowntiumVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(XCRT_RELEASES)));
    QEventLoop event;
...
Version CrowntiumVersionChecker::getMaxReleaseVersion()
{
