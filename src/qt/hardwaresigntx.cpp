...
{
    tool = new CrowntiumHwiTool(this);
    widget = _widget;
}
...
bool HardwareSignTx::askDevice(bool stake, QString* pFingerprint)
{
    ...
    if(!QFile::exists(hwiToolPath))
    {
        ...
        msgBox.setText(tr("HWI tool not found at path \"%1\".<br>Please download it from %2 and add the path to the settings.").arg(hwiToolPath, XCRT_HWI_TOOL));
