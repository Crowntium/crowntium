...
class CrowntiumHwiToolPriv
{
public:
    CrowntiumHwiToolPriv(QObject *parent)
    {
...
CrowntiumHwiTool::CrowntiumHwiTool(QObject *parent) : QObject(parent)
{
    d = new CrowntiumHwiToolPriv(this);
}

CrowntiumHwiTool::~CrowntiumHwiTool()
{
    delete d;
}

bool CrowntiumHwiTool::enumerate(QList<HWDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    devices.clear();
    std::vector<LedgerDevice> vecDevices;
    if(CrowntiumLedger::instance().enumerate(vecDevices, stake))
    {
    ...
    {
        d->strError = QString::fromStdString(CrowntiumLedger::instance().errorMessage());
    }

    return devices.size() > 0;
}

bool CrowntiumHwiTool::isConnected(const QString &fingerprint, bool stake)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    bool ret = CrowntiumLedger::instance().isConnected(strFingerprint, stake);
    if(!ret) d->strError = QString::fromStdString(CrowntiumLedger::instance().errorMessage());
    return ret;
}

bool CrowntiumHwiTool::getKeyPool(const QString &fingerprint, int type, const QString& path, bool internal, QString &desc)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = CrowntiumLedger::instance().getKeyPool(strFingerprint, type, strPath, internal, d->from, d->to, strDesc);
...
bool CrowntiumHwiTool::getKeyPool(const QString &fingerprint, int type, const QString &path, QStringList &descs)
{
    LOCK(cs_ledger);
    bool ret = true;
    QString desc;
...
bool CrowntiumHwiTool::getKeyPoolPKH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::LEGACY, path, descs);
}

bool CrowntiumHwiTool::getKeyPoolP2SH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::P2SH_SEGWIT, path, descs);
}

bool CrowntiumHwiTool::getKeyPoolBech32(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::BECH32, path, descs);
}

bool CrowntiumHwiTool::signTx(const QString &fingerprint, QString &psbt)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    std::string strPsbt = psbt.toStdString();
    bool ret = CrowntiumLedger::instance().signTx(strFingerprint, strPsbt);
    psbt = QString::fromStdString(strPsbt);
    if(!ret) d->strError = QString::fromStdString(CrowntiumLedger::instance().errorMessage());
    return ret;
}

bool CrowntiumHwiTool::signMessage(const QString &fingerprint, const QString &message, const QString &path, QString &signature)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = CrowntiumLedger::instance().signMessage(strFingerprint, strMessage, strPath, strSignature);
    signature = QString::fromStdString(strSignature);
    if(!ret) d->strError = QString::fromStdString(CrowntiumLedger::instance().errorMessage());
    return ret;
}

bool CrowntiumHwiTool::signDelegate(const QString &fingerprint, QString &psbt)
{
...
QString CrowntiumHwiTool::errorMessage()
{
...
bool CrowntiumHwiTool::isStarted()
{
    return d->fStarted;
}

void CrowntiumHwiTool::wait()
{
    if(d->fStarted)
    {
...
bool CrowntiumHwiTool::rescanBlockchain(int startHeight, int stopHeight)
{
    if(!d->model) return false;
...
bool CrowntiumHwiTool::importAddresses(const QString &desc)
{
    if(!d->model) return false;
...
bool CrowntiumHwiTool::importMulti(const QStringList &descs)
{
    bool ret = true;
...
bool CrowntiumHwiTool::finalizePsbt(const QString &psbt, QString &hexTx, bool &complete)
{
    if(!d->model) return false;
...
bool CrowntiumHwiTool::sendRawTransaction(const QString &hexTx, QVariantMap& variantMap)
{
    if(!d->model) return false;
...
bool CrowntiumHwiTool::decodePsbt(const QString &psbt, QString &decoded)
{
    if(!d->model) return false;
...
void CrowntiumHwiTool::setModel(WalletModel *model)
{
    d->model = model;
}

bool CrowntiumHwiTool::execRPC(ExecRPCCommand *cmd, const QMap<QString, QString> &lstParams, QVariant &result, QString &resultJson)
{
    d->strError.clear();
...
void CrowntiumHwiTool::addError(const QString &error)
{
    if(d->strError != "")
        d->strError += "\n";
    d->strError += error;
...
QString CrowntiumHwiTool::derivationPathPKH()
{
    std::string path = CrowntiumLedger::instance().derivationPath((int)OutputType::LEGACY);
    return QString::fromStdString(path);
}

QString CrowntiumHwiTool::derivationPathP2SH()
{
    std::string path = CrowntiumLedger::instance().derivationPath((int)OutputType::P2SH_SEGWIT);
    return QString::fromStdString(path);
}

QString CrowntiumHwiTool::derivationPathBech32()
{
    std::string path = CrowntiumLedger::instance().derivationPath((int)OutputType::BECH32);
    return QString::fromStdString(path);
}
