...
CrowntiumState::CrowntiumState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = CrowntiumState::openDB(_path + "/crtDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

CrowntiumState::CrowntiumState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute CrowntiumState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, CrowntiumTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createCrowntiumAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            crt::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
    ...
    if(voutLimit){
        ...
        return ResultExecute{ex, CrowntiumTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs()), refund.vout.empty() ? CTransaction() : CTransaction(refund)};
    }else{
        return ResultExecute{res, CrowntiumTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs()), tx ? *tx : CTransaction()};
    }
}

std::unordered_map<dev::Address, Vin> CrowntiumState::vins() const // temp
{
...
void CrowntiumState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* CrowntiumState::vin(dev::Address const& _a) const
{
    return const_cast<CrowntiumState*>(this)->vin(_a);
}

Vin* CrowntiumState::vin(dev::Address const& _addr)
{
...
// void CrowntiumState::commit(CommitBehaviour _commitBehaviour)
// {
...
void CrowntiumState::kill(dev::Address _addr)
{
...
void CrowntiumState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void CrowntiumState::deleteAccounts(std::set<dev::Address>& addrs){
...
void CrowntiumState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void CrowntiumState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void CrowntiumState::validateTransfersWithChangeLog(){
...
void CrowntiumState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!CrowntiumState::addressInUse(delegationsAddress)){
        CrowntiumState::createContract(delegationsAddress);
        CrowntiumState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)}, CrowntiumState::version(delegationsAddress));
        commit(CommitBehaviour::RemoveEmptyAccounts);
        db().commit();
    }
}
