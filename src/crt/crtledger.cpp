...
using namespace CrowntiumLedger_NS;

class CrowntiumLedgerPriv
{
public:
    CrowntiumLedgerPriv()
    {
        ...
        if(!toolExists)
        {
            LogPrintf("CrowntiumLedger(): HWI tool not found %s\n", toolPath);
        }
    }
...
CrowntiumLedger::CrowntiumLedger():
    d(0)
{
    d = new CrowntiumLedgerPriv();
}
...
CrowntiumLedger &CrowntiumLedger::instance()
{
    static CrowntiumLedger device;
    return device;
}

bool CrowntiumLedger::signCoinStake(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
...
bool CrowntiumLedger::signBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &vchSig)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool CrowntiumLedger::isConnected(const std::string &fingerprint, bool stake)
{
...
bool CrowntiumLedger::enumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool CrowntiumLedger::signTx(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool CrowntiumLedger::signMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &signature)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool CrowntiumLedger::getKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, std::string &desc)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
std::string CrowntiumLedger::errorMessage()
{
    LOCK(cs_ledger);
...
bool CrowntiumLedger::toolExists()
{
    return d->toolExists;
}

bool CrowntiumLedger::isStarted()
{
    return d->fStarted;
}

void CrowntiumLedger::wait()
{
    if(d->fStarted)
    {
...
bool CrowntiumLedger::beginSignTx(const std::string &fingerprint, std::string &psbt)
{
...
bool CrowntiumLedger::endSignTx(const std::string &, std::string &psbt)
{
...
bool CrowntiumLedger::beginSignBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool CrowntiumLedger::endSignBlockHeader(const std::string &, const std::string &, const std::string &, std::vector<unsigned char> &vchSig)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool CrowntiumLedger::beginEnumerate(std::vector<LedgerDevice> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
    arguments << "enumerate";
...
bool CrowntiumLedger::endEnumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    ...
    for(size_t i = 0; i < jsonDevices.size(); i++)
    {
        ...
        bool isStakeApp = device.app_name == "Crowntium Stake" || device.app_name == "Crowntium Stake Test";
        if(isStakeApp == stake)
        {
...
bool CrowntiumLedger::beginSignMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool CrowntiumLedger::endSignMessage(const std::string &, const std::string &, const std::string &, std::string &signature)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool CrowntiumLedger::beginGetKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, std::string &)
{
    // Get the output type
    std::string descType;
    switch (type) {
...
bool CrowntiumLedger::endGetKeyPool(const std::string &, int, const std::string& , bool, int, int, std::string &desc)
{
    // Decode command line results
...
std::string CrowntiumLedger::derivationPath(int type)
{
