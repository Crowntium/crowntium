...
class CrowntiumDelegationPriv
{
public:
    CrowntiumDelegationPriv():
        m_pfDelegations(0),
    ...
    virtual ~CrowntiumDelegationPriv()
    {
        if(m_pfDelegations)
...
CrowntiumDelegation::CrowntiumDelegation():
    priv(0)
{
    priv = new CrowntiumDelegationPriv();
}
...
bool CrowntiumDelegation::GetDelegation(const uint160 &address, Delegation &delegation) const
{
...
bool CrowntiumDelegation::VerifyDelegation(const uint160 &address, const Delegation &delegation)
{
...
bool CrowntiumDelegation::FilterDelegationEvents(std::vector<DelegationEvent> &events, const IDelegationFilter &filter, int fromBlock, int toBlock, int minconf) const
{
...
std::map<uint160, Delegation> CrowntiumDelegation::DelegationsFromEvents(const std::vector<DelegationEvent> &events)
{
...
void CrowntiumDelegation::UpdateDelegationsFromEvents(const std::vector<DelegationEvent> &events, std::map<uint160, Delegation> &delegations)
{
...
bool CrowntiumDelegation::ExistDelegationContract() const
{
    // Delegation contract exist check
    return globalState && globalState->addressInUse(priv->delegationsAddress);
}

std::string CrowntiumDelegation::BytecodeRemove()
{
    return DelegationABI()["removeDelegation"].selector();
}

bool CrowntiumDelegation::BytecodeAdd(const std::string &hexStaker, const int &fee, const std::vector<unsigned char> &PoD, std::string &datahex, std::string &errorMessage)
{
...
bool CrowntiumDelegation::IsAddBytecode(const std::vector<unsigned char> &data)
{
...
bool CrowntiumDelegation::GetUnsignedStaker(const std::vector<unsigned char> &data, std::string &hexStaker)
{
...
bool CrowntiumDelegation::SetSignedStaker(std::vector<unsigned char> &data, const std::string &base64PoD)
{
    if(!IsAddBytecode(data))
        return false;
