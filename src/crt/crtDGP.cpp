...
void CrowntiumDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool CrowntiumDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule CrowntiumDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t CrowntiumDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t CrowntiumDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t CrowntiumDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t CrowntiumDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool CrowntiumDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void CrowntiumDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void CrowntiumDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void CrowntiumDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void CrowntiumDGP::createParamsInstance(){
...
dev::Address CrowntiumDGP::getAddressForBlock(unsigned int blockHeight){
...
void CrowntiumDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void CrowntiumDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void CrowntiumDGP::parseStorageOneUint64(uint64_t& value){
...
void CrowntiumDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule CrowntiumDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
