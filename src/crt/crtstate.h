...
class CrowntiumTransactionReceipt: public dev::eth::TransactionReceipt {
public:
    CrowntiumTransactionReceipt(dev::h256 const& state_root, dev::h256 const& utxo_root, dev::u256 const& gas_used, dev::eth::LogEntries const& log) : dev::eth::TransactionReceipt(state_root, gas_used, log), m_utxoRoot(utxo_root) {}
...
struct ResultExecute{
    ...
    CrowntiumTransactionReceipt txRec;
    CTransaction tx;
};
...
class CrowntiumState : public dev::eth::State {
    
public:

    CrowntiumState();

    CrowntiumState(dev::u256 const& _accountStartNonce, dev::OverlayDB const& _db, const std::string& _path, dev::eth::BaseState _bs = dev::eth::BaseState::PreExisting);

    ResultExecute execute(dev::eth::EnvInfo const& _envInfo, dev::eth::SealEngineFace const& _sealEngine, CrowntiumTransaction const& _t, dev::eth::Permanence _p = dev::eth::Permanence::Committed, dev::eth::OnOpFunc const& _onOp = OnOpFunc());
    ...
    static const dev::Address createCrowntiumAddress(dev::h256 hashTx, uint32_t voutNumber){
...
struct TemporaryState{
    std::unique_ptr<CrowntiumState>& globalStateRef;
    dev::h256 oldHashStateRoot;
    dev::h256 oldHashUTXORoot;

    TemporaryState(std::unique_ptr<CrowntiumState>& _globalStateRef) : 
...
public:
    ...
    CondensingTX(CrowntiumState* _state, const std::vector<TransferInfo>& _transfers, const CrowntiumTransaction& _transaction, std::set<dev::Address> _deleteAddresses = std::set<dev::Address>()) : transfers(_transfers), deleteAddresses(_deleteAddresses), transaction(_transaction), state(_state){}
...
private:
    ...
    const CrowntiumTransaction& transaction;

    CrowntiumState* state;
