...
#if defined(NDEBUG)
# error "Crowntium cannot be compiled without assertions."
#endif
...
std::unique_ptr<CrowntiumState> globalState;
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        uint64_t minGasPrice = crtDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        uint64_t blockGasLimit = crtDGP.getBlockGasLimit(::ChainActive().Tip()->nHeight + 1);
        size_t count = 0;
        ...
        CrowntiumTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractCrowntiumTX resultConverter;
        if(!converter.extractionCrowntiumTransactions(resultConverter)){
        ...
        std::vector<CrowntiumTransaction> crtTransactions = resultConverter.first;
        std::vector<EthTransactionParams> crtETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(CrowntiumTransaction crtTransaction : crtTransactions){
            sumGas += crtTransaction.gas() * crtTransaction.gasPrice();
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, crtTransaction.gasPrice());
            } else {
                txMinGasPrice = crtTransaction.gasPrice();
            }
            VersionVM v = crtTransaction.getVersion();
            if(v.format!=0)
            ...
            if(crtTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(crtTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(crtTransaction.gas() > UINT32_MAX)
            ...
            if(v.rootVM!=0 && (uint64_t)crtTransaction.gasPrice() < minGasPrice)
...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s XCRT additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    CAmount nSubsidy = 2250 * COIN / blocktimeDownscaleFactor;
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // crt
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // crt
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount){
    ...
    CrowntiumTransaction callTransaction;
    if(addrContract == dev::Address())
    {
        callTransaction = CrowntiumTransaction(nAmount, 1, dev::u256(gasLimit), opcode, nonce);
    }
    else
    {
        callTransaction = CrowntiumTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, nonce);
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<CrowntiumTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path crtDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(crtDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(crtDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(CrowntiumTransaction& tx : txs){
        //validate VM version
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{execRes, CrowntiumTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries()), CTransaction()});
            continue;
        }
...
bool CrowntiumTxConverter::extractionCrowntiumTransactions(ExtractCrowntiumTX& crttx){
    ...
    std::vector<CrowntiumTransaction> resultTX;
    ...
    crttx = std::make_pair(resultTX, resultETP);
    return true;
}

bool CrowntiumTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool CrowntiumTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
CrowntiumTransaction CrowntiumTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    CrowntiumTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = CrowntiumTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = CrowntiumTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t CrowntiumTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // crt
    CrowntiumDGP crtDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setCrowntiumSchedule(crtDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = crtDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = crtDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = crtDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // crt
...
///////////////////////////////////////////////////////////////////////////////////////// crt
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            CrowntiumTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractCrowntiumTX resultConvertCrowntiumTX;
            if(!convert.extractionCrowntiumTransactions(resultConvertCrowntiumTX)){
            ...
            if(!CheckMinGasPrice(resultConvertCrowntiumTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertCrowntiumTX.first, blockGasLimit, pindex->pprev);
            ...
            for(CrowntiumTransaction& qtx : resultConvertCrowntiumTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertCrowntiumTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertCrowntiumTX.first[k].from(),
                        resultConvertCrowntiumTX.first[k].to(),
                        ...
                        resultConvertCrowntiumTX.first[k].getNVout(),
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 5000 && !Params().MineBlocksOnDemand()) {
        ...
        if(pindex->nMoneySupply > (uint64_t)(3000000000 + ((pindex->nHeight - 5000) * 2250)) * COIN){
...
bool CChainState::ConnectTip(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // crt
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // crt
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // crt
            globalState->setRootUTXO(oldHashUTXORoot); // crt
...
bool SignBlockHWI(std::shared_ptr<CBlock> pblock, CWallet& wallet, std::vector<unsigned char>& vchSig)
{
    ...
    CrowntiumLedger &device = CrowntiumLedger::instance();
...
bool SignBlockLedger(std::shared_ptr<CBlock> pblock, CWallet& wallet, std::vector<unsigned char>& vchSig)
{
    ...
    {
        std::string errorMessage = CrowntiumLedger::instance().errorMessage();
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = ::ChainActive().Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // crt
        uint32_t sizeBlockDGP = crtDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Crowntium PoS we need to sync this after every block to ensure txdb is populated for
...
CAmount GetTxGasFee(const CMutableTransaction& _tx)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        CrowntiumTxConverter convert(tx, NULL, NULL, contractflags);
