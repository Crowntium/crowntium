 ...
 */
static const char* BITCOIN_PID_FILENAME = "crtd.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/crowntium/crt>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateCrowntium");
...
bool AppInitBasicSetup()
{
    ...
    _CrtSetReportMode(_XCRT_WARN, _XCRTDBG_MODE_FILE);
    _CrtSetReportFile(_XCRT_WARN, CreateFileA("NUL", GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, 0, 0));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path crtStateDir = GetDataDir() / "stateCrowntium";
                bool fStatus = fs::exists(crtStateDir);
                const std::string dirCrowntium(crtStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsCrowntiumstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<CrowntiumState>(new CrowntiumState(dev::u256(0), CrowntiumState::openDB(dirCrowntium, hashDB, dev::WithExisting::Trust), dirCrowntium, existsCrowntiumstate));
                dev::eth::ChainParams cp(chainparams.EVMGenesisInfo());
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(crtStateDir.string()));
                if (fReset) {
            ...
            try {
                ...
                globalSealEngine->setCrowntiumSchedule(crtDGP.getGasSchedule(::ChainActive().Height() + (::ChainActive().Height()+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
