...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setCrowntiumSchedule(crtDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = crtDGP.getBlockSize(nHeight);
    minGasPrice = crtDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = crtDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    CrowntiumTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractCrowntiumTX resultConverter;
    if(!convert.extractionCrowntiumTransactions(resultConverter)){
    ...
    for(CrowntiumTransaction crtTransaction : crtTransactions){
        ...
        if(bceResult.usedGas + crtTransaction.gas() > softBlockGasLimit){
        ...
        if(crtTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, crtTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= checkpointSpan)
        {
            ...
            crtDelegations.FilterDelegationEvents(events, *this);
            delegations_staker = crtDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                crtDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                crtDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            crtDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            crtDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    CrowntiumDelegation crtDelegations;
    int32_t cacheHeight;
    ...
    void Update(interfaces::Chain::Lock& locked_chain, int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= checkpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                crtDelegations.FilterDelegationEvents(events, *this);
                pwallet->m_my_delegations = crtDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    crtDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                    crtDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                crtDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(crtDelegations.GetDelegation(address, delegation) && CrowntiumDelegation::VerifyDelegation(address, delegation))
                    {
...
public:
...

    {
        ...
        std::string threadName = "crtstake";
...
void StakeCrowntiums(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
