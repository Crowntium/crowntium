...
public:
    ...
    uint32_t nBits;
    uint32_t nNonce;
    uint256 hashStateRoot; // crt
    uint256 hashUTXORoot; // crt
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        READWRITE(nBits);
        READWRITE(nNonce);
        READWRITE(hashStateRoot); // crt
        READWRITE(hashUTXORoot); // crt
    ...
    void SetNull()
    {
        ...
        nBits = 0;
        nNonce = 0;
        hashStateRoot.SetNull(); // crt
        hashUTXORoot.SetNull(); // crt
    ...
    virtual bool IsProofOfStake() const //crt
    {
    ...
    CBlockHeader& operator=(const CBlockHeader& other) //crt
    {
        if (this != &other)
        {
...
public:
    ...
    std::pair<COutPoint, unsigned int> GetProofOfStake() const //crt
    {
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        block.nBits          = nBits;
        block.nNonce         = nNonce;
        block.hashStateRoot  = hashStateRoot; // crt
        block.hashUTXORoot   = hashUTXORoot; // crt
