...
void checkResult(bool isCreation, std::vector<CrowntiumTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    CrowntiumTxConverter converter(transaction, NULL);
    ExtractCrowntiumTX crtTx;
    BOOST_CHECK(converter.extractionCrowntiumTransactions(crtTx));
    std::vector<CrowntiumTransaction> result = crtTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(crttxconverter_tests, TestingSetup)
