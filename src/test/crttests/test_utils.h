...
extern std::unique_ptr<CrowntiumState> globalState;
...
inline void initState(){
    ...
    const std::string dirCrowntium = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<CrowntiumState>(new CrowntiumState(dev::u256(0), CrowntiumState::openDB(dirCrowntium, hashDB, dev::WithExisting::Trust), dirCrowntium + "/crtDB", dev::eth::BaseState::Empty));
...
inline dev::Address createCrowntiumAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline CrowntiumTransaction createCrowntiumTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    CrowntiumTransaction txEth;
    if(recipient == dev::Address()){
        txEth = CrowntiumTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = CrowntiumTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<CrowntiumTransaction> txs){
