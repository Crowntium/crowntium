...
//    initState();
    ...
    txs.push_back(createCrowntiumTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    txs.push_back(createCrowntiumTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    executeBC(txs);

    // Call upgrade to
    dev::Address proxy = createCrowntiumAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<CrowntiumTransaction> txsCall;
    txsCall.push_back(createCrowntiumTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    executeBC(txsCall);

    // Call mint
    std::vector<CrowntiumTransaction> txsMint;
    txsMint.push_back(createCrowntiumTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    auto result = executeBC(txsMint);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);

    // Call balance of
    std::vector<CrowntiumTransaction> txsbalance;
    txsbalance.push_back(createCrowntiumTransaction(ParseHex("70a082310000000000000000000000000101010101010101010101010101010101010101"), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...
//    initState();
    ...
    // Create contract
    std::vector<CrowntiumTransaction> txs;
    txs.push_back(createCrowntiumTransaction(CODE[4], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    executeBC(txs);

    // Call is it constantinople
    dev::Address proxy = createCrowntiumAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<CrowntiumTransaction> txIsItConstantinople;
    txIsItConstantinople.push_back(createCrowntiumTransaction(CODE[5], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
