Gitian building
================
*Setup instructions for a Gitian build of Crowntium Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Crowntium
...
- [Building Crowntium Core](#building-crt-core)
...
Clone the git repositories for crt and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/crowntium/crt --recursive
git clone https://github.com/crowntium/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/crt/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the crt repository under 'Fetch and create inputs' to install sources which require
...
To build Crowntium Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the crt repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/crt/.git/
...
For example:
```bash
URL=https://github.com/laanwj/crt.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit crt=${COMMIT} --url crt=${URL} ../crt/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit crt=${COMMIT} --url crt=${URL} ../crt/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit crt=${COMMIT} --url crt=${URL} ../crt/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-crt:
./bin/gbuild --commit crt=${COMMIT},cpp-eth-crt=${ETHCOMMIT} --url crt=${URL},cpp-eth-crt=${ETHURL} ../crt/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the crt git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../crt/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/crowntium/crt-detached-sigs.git

BTCPATH=/some/root/path/crt
SIGPATH=/some/root/path/crt-detached-sigs

./bin/gbuild --url crt=${BTCPATH},signature=${SIGPATH} ../crt/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/crt-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/crt-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/crt-osx-build.assert
```
...
[crowntium/gitian.sigs](https://github.com/crowntium/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@crt.org) and he will commit them.
